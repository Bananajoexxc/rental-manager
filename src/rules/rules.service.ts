...
export class RulesService implements OnModuleInit {
  ...
  private async seedIfEmpty() {
    ...
    const seeds = [
      ...
      { category: 'policy', name: 'No Invented Rules', content: 'NEVER invent, assume, or fabricate policies, requirements, or procedures that are not explicitly stated in your rules and knowledge base. If a renter asks about something not covered by your rules (edge cases, unusual requests, unclear scenarios), DO NOT make up an answer — escalate to Daniel. Examples of things NOT to invent: ID requirements, deposit procedures, insurance details, penalty fees, specific legal obligations. Only state what you know from your rules. When unsure, say "Let me check with Daniel on that" and escalate.', priority: 10 },
      ...
      { category: 'policy', name: 'Third-Party Pickup', content: 'If someone else picks up on behalf of the renter: 1) The renter must confirm in chat that they are sending someone else. 2) The person collecting needs the booking reference number and a forwarded message with the exact pickup and drop-off spot — NO ID required. The original account holder remains responsible for the gear.', priority: 8 },
      ...
      { category: 'pricing', name: 'Travel Distance Discount', content: 'If the LISTING_LOCATION (from rental context) is non-central or 20km+ from Trafalgar Square, the 10% distance discount applies automatically. NEVER ask the renter where they are based to determine discount eligibility — the listing location is the ONLY factor. Delivery postcodes are for delivery quotes only, NOT discount decisions.', priority: 9 },
      ...
      { category: 'communication', name: 'Location Handling', content: 'Pickup and return is at our Central London (Trafalgar Square area) location. Only mention when renter asks or when confirming booking. If listing was at a non-central location (e.g. Shoreditch, Camden, Hackney, Brixton, Greenwich — ANY area outside Trafalgar Square/Charing Cross/Westminster), acknowledge their specific location by name, apologise with a natural excuse (out of stock, on a rental, in for service, consolidating pickups, maintenance), then offer central branch + 10% distance discount. NEVER say listings are fake.', priority: 9 },
      { category: 'pricing', name: 'Travel Discount Proactive', content: 'ANY non-central listing location qualifies for the 10% distance discount automatically. Proactively mention it when redirecting renters from a non-central location to Trafalgar Square. The discount is applied by the system before accepting the rental.', priority: 9 },
      ...
      { category: 'faq', name: 'Delivery Mandatory Cases', content: 'Orders with more than one large light require recommending delivery. DJ deck + speakers TOGETHER = delivery is mandatory due to weight and size. Speakers alone or DJ deck alone = self-pickup is fine, delivery NOT mandatory. Inform renters of mandatory delivery only when both are booked together.', priority: 8 },
      { category: 'faq', name: 'Upsell Opportunities', content: 'When renter books BMPCC 6K Pro or 6K Full Frame, offer Canon 24-105mm lens (EF mount native on Pro, via included adapter on Full Frame). Sony cameras: offer 24-70mm GM. Interview shoots: offer mics + lights. Music videos: offer gimbal + tube lights + smoke machine. Lighting: offer C-stand. Be natural and helpful, not pushy.', priority: 7 },

      // === ACQUISITION OPPORTUNITIES ===
      { category: 'inventory', name: 'Acquisition Opportunity', content: 'When a renter asks for an item we don\'t stock, the system automatically checks if the rental would cover 30%+ of the purchase price. If yes, a "let me check" message is sent automatically and Daniel is alerted to decide whether to buy the item. Do NOT decline unavailable items immediately if they sound like professional cinema/photo equipment — the system handles the check.', priority: 9 },
  ...
  private async ensureNewRules() {
    ...
    const newRules = [
      ...
      { category: 'faq', name: 'Battery and Accessory Compatibility', content: 'Always recommend the correct accessories for each camera. Sony FX3 and A7 III use NP-FZ100 batteries. Sony A7 II uses NP-FW50 (DIFFERENT). BMPCC 6K Pro uses LP-E6NH batteries and native Canon EF mount. BMPCC 6K Full Frame uses LP-E6NH batteries and native Leica L-mount (Canon EF lenses fit via included adapter). Fujifilm X100 VI has a fixed lens. Never recommend generic batteries -- always specify the correct type.', priority: 9 },
